...
const char RACESYNC_CAMERA_UI[] PROGMEM = R"HTML(<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>RaceSync Camera</title><style>:root{font-family:system-ui,-apple-system,sans-serif;color:#f4f6f8;background:#101418}body{margin:0}.wrap{max-width:820px;margin:auto;padding:18px}.panel{background:#192027;border:1px solid #2a343d;border-radius:12px;padding:16px;margin-bottom:12px}.btn{border:0;border-radius:8px;padding:10px 14px;margin:4px;font-weight:700;background:#2f7d4a;color:#fff}.btn.alt{background:#a33b3b}.btn.dim{background:#3c4852}.btn:disabled{opacity:.45}.nav{color:#fff;text-decoration:none;margin-right:12px}.warn{color:#ffd479}.ok{color:#8ee2a8}.bad{color:#ff8c8c}.diag{font-size:.9rem;color:#aab8c2}.cameraState{font-size:1.1rem;font-weight:800;margin-top:12px}pre{white-space:pre-wrap;word-break:break-word;color:#dbe7ef}</style></head><body><main class="wrap"><div class="panel"><h2>GoPro HERO9 Bluetooth</h2><a class="nav" href="/">Sessions</a><a class="nav" href="/status">Status</a><a class="nav" href="/control">Control</a></div><div class="panel"><p class="warn">Bluetooth camera control is separate from the RaceSync logging path. A GoPro failure must never prevent VBO recording.</p><button type="button" class="btn" id="connect">Enable Bluetooth &amp; Connect</button><button type="button" class="btn dim" id="refresh">Refresh status</button><button type="button" class="btn alt" id="disconnect">Disconnect</button><p class="cameraState" id="cameraState">Camera status loading…</p><p id="notice">Loading…</p><p class="diag" id="clientDiag">Client ready; no camera command sent yet.</p><pre id="status"></pre></div></main><script>
let busy=false;
let commandCount=0;
...
async function load(){
  try{
    ...
    cameraState.textContent='RaceSync unavailable';
    cameraState.className='cameraState bad';
    notice.textContent='Unable to read camera status';
...
function show(j,ok){
  ...
  if(j.recording){
    ...
    cameraState.textContent='GoPro connected — not recording';
    cameraState.className='cameraState ok';
  }else{
    cameraState.textContent='GoPro disconnected';
    cameraState.className='cameraState warn';
  }
  ...
  if(j.lastError){
    ...
    notice.textContent='Camera controls locked while RaceSync is logging; status shown from RAM only';
